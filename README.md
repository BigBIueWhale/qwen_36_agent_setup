...
Empirically reproduced (2026-04-28). Sequence: step 1 (`prompt 153,666 tokens > 152,000` + image) → HTTP 400 in 0.46 s; step 2 (clean small request, same image) → **HTTP 500 in 0.06 s** with the receiver-cache assertion in engine logs; step 3 (different image, large again) → HTTP 500 plus `/health=503` (engine degraded). Class A — vLLM is breaking its own sender↔receiver cache invariant on a path it owns end-to-end.
...
### 6.10 Mistral-only flag drives parser-agnostic-shaped dispatch sites [Class A — vLLM internal inconsistency, sidestepped]

`vllm/entrypoints/openai/chat_completion/protocol.py:401` declares `_grammar_from_tool_parser: bool = PrivateAttr(default=False)` with the docstring "CAUTION: Should only be set by `ToolParser.adjust_request`" — i.e., parser-agnostic by name and docstring. But all four readers in `chat_completion/serving.py` (lines 329, 554, 823, 1014, 1407) dispatch into Mistral-specific code. The streaming-side branch at line 823 calls `MistralToolParser.extract_maybe_reasoning_and_tool_streaming(...)` and asserts `isinstance(tool_parser, MistralToolParser)` — the assertion is the **authored contract** of PR #39217 (`c0722f22d`, [Mistral Grammar] Fix tool and reasoning parsing, Julien Denize @ Mistral). The non-streaming branch at line 1407 calls `MistralToolParser.build_non_streaming_tool_calls(tool_calls)` **without** the matching `isinstance` guard — a latent symmetric bug. The correct pattern lives at `vllm/entrypoints/openai/engine/serving.py:619`, which uses `is_mistral_tool_parser(tool_parser_cls) AND request._grammar_from_tool_parser` from the `vllm.utils.mistral` helper; the chat-completion serving path was simply not migrated to that pattern.

**Affects us**: indirectly. Our §7.7 patch deliberately does **not** set the flag from `Qwen3CoderToolParser.adjust_request`, sidestepping both unguarded sites. So §6.10 is **not weaponized in this deployment** — Phase 9 case 1 of the §7.7 patch positively asserts the flag remains `False` after our `adjust_request` runs. The bug is documented here so a future maintainer who re-introduces the flag write (e.g., when borrowing from another parser as an idiom) understands why the writer is forbidden, and so any upstream fix can be tracked. **No patch in this repo addresses §6.10**; the upstream remedy (extending the line 619 pattern to lines 823 and 1407) is the proper fix and belongs in upstream vLLM. **Removal trigger for §7.7's "do not set flag" invariant**: vLLM upstream guards the streaming dispatch at `chat_completion/serving.py:823` and the non-streaming dispatch at `:1407` on `is_mistral_tool_parser(type(tool_parser))`, mirroring `engine/serving.py:619`. When that lands, setting the flag from a non-Mistral parser becomes safe again — though semantically still wrong (we don't want Mistral helpers running on Qwen3 output), so we keep §7.7's writer-side restraint regardless.

### 6.11 — `tool_choice="none"` ignored on the streaming chat-completion path [Class A — vLLM internal inconsistency, NOT WEAPONIZED]

When the client sends `{"tool_choice":"none","tools":[…non-empty…],"stream":true}` and the model emits XML tool-call markup as content, the streaming path silently parses the XML into structured `tool_calls` deltas and returns `finish_reason="tool_calls"`. The non-streaming path on the same prompt returns `tool_calls:[]`, the XML preserved as `content`, and `finish_reason="stop"`. The two paths disagree on a contract the spec is unambiguous about (`"none" = no tool calls`).

**Cited source.** The streaming side has no `tool_choice` gate: `Parser.parse_delta` at `vllm/parser/abstract_parser.py:585-647` calls `extract_tool_calls_streaming` whenever `_in_tool_call_phase(state)` is true; the cascade at `vllm/entrypoints/openai/chat_completion/serving.py:853-1010` falls into the `parser is not None` branch at `:999-1005` for `tool_choice="none"` because the named/required conditions all evaluate False. The non-streaming side gates explicitly: `_parse_tool_calls_from_content` at `vllm/entrypoints/openai/engine/serving.py:606-720` has an explicit `tool_choice="none"` non-match (no `auto`/`required`/named branch fires); `chat_completion/serving.py:1504` then builds a plain `ChatMessage(content=content)`.

**Affects us**: no. qwen-code 0.15.6 unconditionally calls vLLM with `stream:true` AND never writes `tool_choice` on the request body — the field is absent (verified by source-level audit of `pipeline.ts:324-329`, by line-by-line audit of the bundled `cli.js`, and by transparent-proxy capture of two real outgoing tool-using requests stored at `/tmp/safety_repro/captures_tool/`). Both halves of the trigger (`tool_choice="none"` AND `stream:true`) need to coexist; neither half ever fires from our agentic loop. **No patch in this repo addresses §6.11**; the upstream remedy (consult `request.tool_choice` in `Parser.parse_delta`) is the right fix and belongs in upstream vLLM.

**For client authors who DO want to use `tool_choice="none"`**: pick one of:
- Use `stream:false`. The non-streaming path's gate fires correctly: `content` is the verbatim model output, `tool_calls=[]`, `finish_reason="stop"`.
- OR strip the `tools` array from the request when `tool_choice="none"`. Without tools defined the parser has nothing to extract, so the bug becomes unreachable.
- OR validate `finish_reason` and `tool_calls` client-side: when you sent `tool_choice="none"`, treat any `tool_calls` in the response as the model's *suggestion only* (not a sanctioned call) and read `content` (which the streaming path will include — see §6.13's converse — even if it's empty in the bug-shape; in practice the XML lives in the synthesised `tool_calls.arguments`).

### 6.12 — `tool_choice="required"` silently drops model text on the non-streaming path [Class A — vLLM internal inconsistency, NOT WEAPONIZED]

The inverse asymmetry of §6.11. When the client sends `{"tool_choice":"required","tools":[…],"stream":false}` and the model declines to emit a tool call (e.g., the prompt didn't actually invite one and the model responds with "I can't help with that"), the non-streaming path silently sets `content=""` and forces `finish_reason="tool_calls"`. The model's actual text is discarded with no error; a client that reads only `content` sees an empty string. The streaming path on the same prompt preserves the text via `content` deltas — i.e., for `tool_choice="required"` with a non-tool-emitting model, **streaming is strictly more correct than non-streaming**.

**Cited source.** `vllm/entrypoints/openai/chat_completion/serving.py:1497` hard-codes `content=""`; lines 1576-1580 then force `finish_reason="tool_calls"` regardless of whether any tool call was actually parsed. The streaming side has no equivalent override — content deltas pass through to the response naturally.

**Affects us**: no. qwen-code never sets `tool_choice="required"` (same audit as §6.11). Our smoke tests (`/tmp/smoke_test/verify_v2.sh`) include a `tool_choice="required"` curl, but the model in that test always emits the requested tool call — the trigger requires the model declining to comply, which our tests don't exercise. **No patch in this repo addresses §6.12**; the upstream remedy (preserve content in `serving.py:1497`, or surface a typed error) is the right fix and belongs in upstream vLLM.

**For client authors who DO want to use `tool_choice="required"`**: pick one of:
- Use `stream:true`. The streaming path preserves the model's text via `content` deltas correctly.
- OR after a `stream:false` `tool_choice="required"` response, if `tool_calls` is empty AND `content==""` AND `finish_reason=="tool_calls"`, retry the request **without** `tool_choice="required"` (or with `tool_choice="auto"`) to recover the model's intent. The retry will reveal whether the model refused, asked a clarifying question, or had a transient issue.

### 6.13 — Named `tool_choice` returns wrong `finish_reason` on the non-streaming path [Class A — minor; contradicts adjacent code comment, NOT WEAPONIZED]

When the client sends `tool_choice={"type":"function","function":{"name":"X"}}` (i.e., "you must call this specific tool"), the non-streaming response returns `finish_reason="stop"`; the streaming response returns `finish_reason="tool_calls"`. The two paths disagree. The comment at `vllm/entrypoints/openai/chat_completion/serving.py:1573-1575` says the non-streaming value should be `"tool_calls"` — the code contradicts its own comment.

**Cited source.** `vllm/entrypoints/openai/chat_completion/serving.py:1573-1575` (comment + non-streaming path). The streaming path at `:853-1010` produces `"tool_calls"` consistently with the rest of the streaming-side conventions.

**Affects us**: no. qwen-code never uses named `tool_choice`. The actual tool call is correctly produced in both modes; only the `finish_reason` field disagrees.

**For client authors who DO want to use named `tool_choice`**: do NOT branch on `finish_reason` to decide whether the named tool was called. Inspect `tool_calls[0].function.name == "X"` directly. The tool-call shape itself is correct in both transports; only the closure metadata is inconsistent.

### 6.14 — Streaming silently drops tool-argument content on `max_tokens` truncation [Class A — vLLM internal inconsistency, addressed by §7.11]

When `max_tokens` truncates the model's output INSIDE a `<parameter=KEY>VALUE` block — no `</parameter>` reached, no sibling `<parameter=`, no `</function>`, no `</tool_call>` reached either — the streaming parser silently drops the in-flight value entirely. The non-streaming parser preserves it. Concretely: a `write_file` call with a `content` parameter that exceeds `max_tokens` produces, on streaming, `arguments='{"file_path":"out.py"'` (just the first parameter; the entire `content` value lost), versus non-streaming on the same prompt returning the partial `content` value intact.

**Cited source.** `Qwen3CoderToolParser.extract_tool_calls_streaming` at `vllm/tool_parsers/qwen3coder_tool_parser.py:550-573`: the loop searching for parameter delimiters falls through every alternative (no `</parameter>`, no `<parameter=`, no `</function>`, no `</tool_call>` in the buffer) and `break`s without emitting any json fragment for the partial value. The non-streaming counterpart at `vllm/tool_parsers/qwen3coder_tool_parser.py:66-69` uses the regex `<parameter=(.*?)(?:</parameter>|(?=<parameter=)|(?=</function>)|$)` whose `|$` alternative captures up to EOF — so the partial value survives. The serving-layer fixup `_create_remaining_args_delta` at `vllm/entrypoints/openai/chat_completion/serving.py:1107-1158` does NOT rescue this case because its `should_check` predicate requires `delta_message.tool_calls[0].function.arguments is not None`, but the broken streaming parser returns `None` from `extract_tool_calls_streaming` when truncation lands mid-value.

This is independent of speculative decoding (the spec-decode-related defenses at lines 499-503, 526-532, 627-632 are not implicated; we don't enable `--speculative-config` anyway). The bug is purely in the streaming state machine's missing flush-on-EOS for in-flight values.

**Affects us**: yes-if-triggered. With `max_tokens=32000` (`docker/config/settings.json:39`) and the agent's `write_file`, `replace`, and `run_shell_command` tools — all of which take string parameters that can grow large — any prompt asking the agent to produce a single tool call with > ~30 000 tokens of arguments (e.g., "write me one big file") will silently lose the partial content. Rare in our typical workflow but real, especially because qwen-code unconditionally streams (see §5.5). **Resolution**: §7.11 — `monkey_patch_qwen3_coder_streaming_truncation.py` keeps `prev_tool_call_arr[idx]["arguments"]` in sync with the in-progress partial AND extends the serving-layer rescue predicate so the existing `expected_call − actual_call` math at `serving.py:1149-1158` emits the missing-tail bytes as a final delta.

**For client authors who CANNOT take this patch** (e.g., third-party deployments running stock vLLM): defend client-side. After every streaming tool-call response, before passing `tool_calls[i].function.arguments` to a JSON parser, check `usage.completion_tokens >= max_tokens` AND `finish_reason == "tool_calls"` — if both, treat the call as truncated and either (a) retry with a larger `max_tokens`, or (b) repeat the request with `stream:false` so the regex-based parser captures the partial value, or (c) surface a typed truncation error to the upstream caller.

---

## 7. The patches in this repo

Eleven monkey-patches plus one container-entrypoint launcher plus one sitecustomize loader. Every patch is **server-side**, loaded into the vLLM Python process by `launch_with_patches.py` (in PID 1) and re-loaded by `sitecustomize.py` in spawned EngineCore subprocesses (load-bearing for patches 3 and 8; see §7.S). There is no client-side code in this repo.
...
| 7 | [`monkey_patch_qwen3_coder_grammar.py`](monkey_patch_qwen3_coder_grammar.py) | §5.6 / agentic-correctness — server-side xgrammar `structural_tag` constraint on tool emission with `JSONSchemaFormat(style="qwen_xml")` body enforcement; flips `supports_required_and_named=False` to close a latent JSON-list-path bug |
...
| 11 | [`monkey_patch_qwen3_coder_streaming_truncation.py`](monkey_patch_qwen3_coder_streaming_truncation.py) | D13 — streaming-mode `Qwen3CoderToolParser` silently drops the partial parameter value when `max_tokens` truncates inside `<parameter=KEY>VALUE`; non-streaming preserves it via the regex's `\|$` alternative. Patch keeps `prev_tool_call_arr` in sync with the in-progress partial AND extends the serving-layer rescue predicate to fire on the truncation case so the existing `expected_call − actual_call` math emits the missing tail as a final delta |
| 12 | [`monkey_patch_repetition_detection_default.py`](monkey_patch_repetition_detection_default.py) | §7.12 — server-side default for `SamplingParams.repetition_detection` (vLLM PR #35451 hard-stop on n-gram emission loops); fills the field iff the client did not explicitly send it. Necessary because vLLM's `--override-generation-config` allow-list (`vllm/config/model.py:1427`) drops `repetition_detection` and `to_sampling_params` (`protocol.py:596`) passes the request value through verbatim — no built-in default mechanism reaches this field |
| L | [`launch_with_patches.py`](launch_with_patches.py) | Container entrypoint that imports the 12 patches in order, runs per-patch verification, then hands off to `vllm.entrypoints.cli.main` via `runpy.run_module(alter_sys=True)` |
...
Wraps **both** `Qwen3ReasoningParser.extract_reasoning` (non-streaming) **and** `Qwen3ReasoningParser.extract_reasoning_streaming`, emitting a single structured WARNING (`model_emit_warning kind=tool_call_in_reasoning reasoning_len=N marker_count=M`) whenever the reasoning half of a request contains literal `<tool_call>` markup. The two wrappers funnel through one shared `_WARNING_FORMAT` constant, so the WARNINGs are byte-identical regardless of delivery modality and a single forwarder regex (§8.4.2) covers both code paths. Both wrappers pass the upstream return value through **unchanged** — the agent's retry policy decides what to do. Detect, don't rescue: the parser is correct to its contract, the model is misbehaving, and rescuing would require a state-machine bandage. The streaming wrapper accumulates a per-request reasoning buffer on the parser instance (vLLM constructs a new instance per request at `vllm/entrypoints/openai/chat_completion/serving.py:247`, so buffer + dedup-flag reset implicitly between requests) and dedups so each request fires at most one WARNING. The patch's Phase 3 import-time self-test drives the streaming wrapper through six synthesised cases (None passthrough, no-marker accumulation, single-delta marker, marker split across deltas, intra-request dedup, content-only delta) before any real request reaches the engine. **Removal trigger**: Qwen3.6 retraining eliminates the OOD emission.
...
1. **Tool-emission grammar.** With `--tool-call-parser qwen3_coder` and the default `tool_choice="auto"`, the inherited `ToolParser.adjust_request` (`vllm/tool_parsers/abstract_tool_parser.py:85-122`) is a no-op — `get_json_schema_from_tools` returns None for `"auto"`. The model emits XML tool calls **completely unconstrained**; a post-hoc parser then extracts what it can. The patched `adjust_request` builds an xgrammar `structural_tag` payload — modern `{"type":"structural_tag","format":{"type":"triggered_tags",...}}` shape, one tag per registered tool — mapping `<tool_call>\n<function=NAME>\n` → per-tool body grammar → `\n</function>\n</tool_call>` from `request.tools` and installs it on `request.structured_outputs`. The body grammar is compiled from each tool's JSON Schema with `JSONSchemaFormat(style="qwen_xml")` (`xgrammar/structural_tag.py:35-67`), which xgrammar shipped in 0.1.32 — the lower bound of vLLM's `xgrammar>=0.1.32,<1.0.0` pin (`vllm/requirements/common.txt`). The `qwen_xml` style implements Qwen3-Coder's **hybrid wire format**: XML framing (`<parameter=KEY>VALUE</parameter>`) on the outside, with VALUE encoded per-type — raw text for `type:"string"` properties (no JSON quoting; the chat template at `chat_template.jinja:122` renders strings as `args_value | string`, omitting `tojson`), and JSON literals for everything else (numbers, booleans, arrays, objects; the chat template uses `args_value | tojson`). xgrammar's `qwen_xml` style enforces both halves: the XML framing (literal `<parameter=` prefix, `</parameter>` suffix) is mandatory, the body content type-checks against the schema (e.g. `"42"` is rejected for `type:integer`, `42` is accepted; quoted/unquoted strings are both accepted because both are valid string content). **It does NOT set `request._grammar_from_tool_parser = True`** — see the dedicated paragraph below for why; that flag's contract is Mistral-format-specific despite its parser-agnostic-sounding name. **What the constraint enforces**: (a) **function-name pinning** — the function name on the wire is always one of the registered tools; xgrammar's literal `<function=NAME>` framing makes hallucinated names structurally unreachable; (b) **framing integrity** — the `<tool_call>...</tool_call>` and `<function=…>...</function>` markers cannot be elided or duplicated; (c) **parameter-body shape (Qwen XML)** — each parameter must be emitted as `<parameter=KEY>VALUE</parameter>` against the registered JSON Schema, with `required` propagated and value types enforced. Empty bodies, JSON-shaped bodies, missing `required` parameters, and type-mismatched values are all rejected at decode time. xgrammar's bitmask FSM stays dormant during `<think>...</think>` (`enable_in_reasoning: bool = False` default at `vllm/config/structured_outputs.py:41`) and engages only after `</think>`, so thinking-mode reasoning remains unconstrained. Per-request overhead xgrammar paper-measured at <6% TPOT.

   **v2 history (May 2026): the body-style fix.** Earlier (v1) versions of this patch built the structural_tag using xgrammar's deprecated `structures`+`triggers` JSON shape. That shape silently routed through `xgrammar/structural_tag.py:506-535`'s `from_legacy_structural_tag` converter, which hardcodes `JSONSchemaFormat(style="json")` — the legacy API has no way to express XML body enforcement. xgrammar consequently forced the model into JSON output between `<function=NAME>` and `</function>`, contradicting Qwen3-Coder's training-shape XML and the post-hoc `Qwen3CoderToolParser._parse_xml_function_call` regex (`vllm/tool_parsers/qwen3coder_tool_parser.py:225-253`) which scans for `<parameter=KEY>VALUE</parameter>`. The parser matched zero `<parameter=` markers in JSON output, set `param_dict={}`, and emitted `arguments="{}"` for every single tool call. In an agentic loop the model could not escape — even self-aware re-prompting (the model's thinking explicitly said "I'm having issues with my tool calls. Let me try a different approach and be very explicit about the parameters") couldn't fight the FSM masking every non-`{` token. v2 emits the modern `StructuralTag`+`triggered_tags` shape with explicit `style="qwen_xml"`, recovered the body enforcement, and added a Phase 9 byte-walk regression guard that asserts the matcher rejects `{` at body-open and accepts `<parameter=…>…</parameter>`.

   **Why we deliberately do NOT set `request._grammar_from_tool_parser = True`** — and why that's load-bearing for streaming + non-streaming tool calls. The `_grammar_from_tool_parser` PrivateAttr's docstring at `protocol.py:401-402` reads parser-agnostic ("CAUTION: Should only be set by `ToolParser.adjust_request`"), but its actual contract is Mistral-only. Both the field AND its readers were introduced in a single commit — vLLM PR #39217 ([Mistral Grammar] Fix tool and reasoning parsing, Julien Denize @ Mistral). The streaming-path consumer at `chat_completion/serving.py:823` enters a branch that calls `MistralToolParser.extract_maybe_reasoning_and_tool_streaming(...)` and asserts `isinstance(tool_parser, MistralToolParser)` — the assertion is the **authored contract**, not defensive scaffolding. The non-streaming consumer at `chat_completion/serving.py:1407` calls `MistralToolParser.build_non_streaming_tool_calls(tool_calls)` without even an `isinstance` guard. Neither method exists on `Qwen3CoderToolParser` (verified by `grep` at the pinned commit). If we set the flag from our non-Mistral parser, the streaming path returns HTTP 500 from the assertion and the non-streaming path returns HTTP 500 from `AttributeError`. The structural_tag enforcement we want is **independent** of the flag: `vllm/v1/structured_output/backend_xgrammar.py:92-106` consumes `request.structured_outputs.structural_tag` directly. Phase 5's landmark check still validates that the `_grammar_from_tool_parser` PrivateAttr exists on `ChatCompletionRequest` — its removal would mean upstream rewrote the Mistral-grammar plumbing, and our "do not set the flag" invariant must be re-audited. (Earlier versions of this patch did mirror Mistral by setting the flag; that produced broken streaming tool calls until the writer was removed. The patch's docstring + the launcher's Phase 5 verifier now encode "do not set this" as a positive invariant. See §6.10 for the analogous unguarded upstream bug at line 1407 that we sidestep but don't patch.)
...
The override is gated: skipped if `request.structured_outputs` or `request.response_format` is already set (never tramples explicit client intent), if `tool_choice == "none"`, or if the request is a `ResponsesRequest` (which uses `request.text` rather than `request.structured_outputs`). Phase 9 behavioural verification at import time constructs a real `ChatCompletionRequest` with a synthetic single-tool list, calls the wrapped `adjust_request`, parses the resulting `structural_tag` JSON, and asserts: (a) **legacy-shape rejection** — top-level `"structures"` key MUST be absent (its presence routes through `from_legacy_structural_tag` and hardcodes `style="json"`, the v1 bug); (b) **modern-shape adherence** — `type=="structural_tag"`, `format.type=="triggered_tags"`, every tag has `content.style=="qwen_xml"`; (c) **build-time compile** — `xgrammar.Grammar.from_structural_tag(tag_str)` succeeds; (d) **functional regression guard** — a synthetic `xgr.GrammarCompiler` + `xgr.GrammarMatcher` over a 256-byte vocab byte-walks the canonical `<parameter=expr>…</parameter>` body and asserts every byte is accepted, then walks to the body-open boundary and asserts `{` is rejected. This is the load-bearing proof: schema-shape verification alone passed for the v1 buggy payload too — only the byte-walk catches "compiles, but enforces the wrong shape." Three negative controls (empty tools, explicit client `structured_outputs`, `tool_choice="none"`) prove the gate fires.
...
### 7.11 Patch 11 — `monkey_patch_qwen3_coder_streaming_truncation.py`

Closes the **D13 streaming-vs-non-streaming asymmetry**: `Qwen3CoderToolParser.extract_tool_calls_streaming` (`vllm/tool_parsers/qwen3coder_tool_parser.py:326-683`) silently drops the partial parameter value bytes when `max_tokens` truncates the model output INSIDE a `<parameter=KEY>VALUE` block. Concretely, the inner-loop block at lines 550-573 hits its `# Parameter incomplete — break` fallback when `value_text` contains none of `</parameter>`, `<parameter=`, `</function>`, `</tool_call>` — the parser breaks without ever emitting a json fragment for the partial. The serving-layer rescue at `serving.py:1107-1158` cannot recover the partial because `_should_check_for_unstreamed_tool_arg_tokens` (lines 1785-1807) requires `delta_message.tool_calls[0].function.arguments is not None`, which is False on the truncation path (parser returns None; line 1044 substitutes `DeltaMessage()` with `tool_calls=[]`).

The non-streaming counterpart `extract_tool_calls` uses the regex `<parameter=(.*?)(?:</parameter>|(?=<parameter=)|(?=</function>)|$)` whose terminal `|$` alternative captures up to EOF on truncation, so non-streaming preserves the partial value bytes. Concrete observed shapes (`/tmp/safety_repro/D13_force_truncate.sh`):

```
NON-STREAM (correct, 681 bytes):  arguments='{"file_path": "long_script.py", "content": "#!/usr/bin/env python3\\n...\\nimport os\\nimport sys"'
STREAM   (broken,    30 bytes):  arguments='{"file_path": "long_script.py"'
```

Streaming silently drops the entire `content` parameter — 651 bytes of model output evaporated.

**Patch design.** Three moving parts split across two layers:

1. **Parser-side wrap** — `Qwen3CoderToolParser.extract_tool_calls_streaming`. After the original returns, if the parser is in mid-parameter-value state (`is_tool_call_started=True, in_function=True, json_started=True, json_closed=False`), recompute the would-be `expected_call` JSON for the in-progress tool call and store it in `prev_tool_call_arr[current_tool_index]["arguments"]`. The serving-layer rescue at `serving.py:1139-1145` reads that slot as `expected_call`; keeping it accurate is what makes the rescue produce the right final delta. Idempotent on the happy path: the parser's existing happy-path update at lines 642-657 fires when `</function>` is observed and overwrites our intermediate value with the byte-identical full-args JSON the non-streaming `_parse_xml_function_call` produces. Our wrap is only load-bearing when `</function>` never arrives. The wrap **never** mutates `streamed_args_for_tool`; that array remains the truthful record of what was actually emitted on the wire.

2. **Serving-side wrap** — `OpenAIServingChat._should_check_for_unstreamed_tool_arg_tokens`. Calls the original; returns True if it was True. Else, on the truncation rescue case (`output.finish_reason is not None`, `self.enable_auto_tools`, `self.tool_parser is Qwen3CoderToolParser`, `delta_message is not None`), the wrap **mutates** `delta_message` to add a synthetic `DeltaToolCall(index=0, function=DeltaFunctionCall(arguments=""))` so the rescue body's unconditional `delta_message.tool_calls[0]` indexing at `serving.py:1116-1126` does not raise IndexError, and returns True so the rescue body executes. The mutation is a deliberate side effect inside what looks like a pure predicate; it is the smallest possible site that runs **before** the rescue body's unconditional indexing — wrapping the 700-line `chat_completion_stream_generator` itself would be the alternative and offers no semantic benefit. The synthetic skeleton's id/type/name are None; `_create_remaining_args_delta` at `serving.py:1819-1836` is already tolerant of that (`original_tc.id if original_tc else None`).

3. **Helper method** — `Qwen3CoderToolParser._qwen36_compute_partial_args_json`. Reproduces the parser's own param-locating logic on the cumulative `current_text` to identify the in-progress parameter and serialize its partial value via the same `_convert_param_value` / `json.dumps` path the loop body uses. Returns the full would-be `expected_call` JSON string (no closing `}`, matching non-stream truncation byte-shape — see `extract_tool_calls`' regex at line 235 with the `|$` alternative that captures the partial without forcing a JSON closer) or `None` if no partial is observable.

**Why not Option A (full method override).** The streaming method is 358 lines of state machine. Replacing it verbatim doubles the bug-discovery surface for any future upstream refactor; the parser's MRO walk and source landmarks already let us detect surrounding-code drift, and the small wrap composes with that machinery instead of re-implementing it.

**Why not Option C alone (serving-layer rescue extended to read parser state).** The serving-layer rescue reads `tool_parser.prev_tool_call_arr[index].get("arguments", {})`. If the parser doesn't update that slot with the partial, the rescue computes `expected = "{}"` (the initial header value at parser line 472) and `remaining = "{}".replace(streamed, "")` which yields `"{}"` (because `streamed` is not a substring of `"{}"`); the spurious final delta emits a literal `"{}"` after what the wire already saw. Either we update the parser's state OR we replace the rescue body wholesale. The former is one wrap on the parser; the latter is wrapping a 700-line generator. We update.

Phase 10 behavioural verification at import time drives the patch through four cases: **case A** drives a 5-chunk truncation sequence through the patched parser end-to-end and asserts that `prev_tool_call_arr[-1]["arguments"]` contains the file_path AND the partial content value bytes (`"import os"` / `"import sys"`) with no spurious closing brace; **case B** re-runs the same chunk sequence through the captured original (bypassing the wrap) and asserts the unpatched parser leaves the partial value out of `prev_tool_call_arr` — proving the bug landmark still fires; **case C** drives the predicate wrap through four sub-cases (original True passthrough, truncation rescue with synthetic-skeleton mutation, other parser class extension does NOT fire, no-finish_reason extension does NOT fire); **case D** verifies the end-to-end rescue math (`expected.replace(actual, "", 1)` yields exactly the missing-tail bytes the wire is supposed to receive — strict-prefix difference, `actual + remaining == expected`).

A half-installed truncation rescue is strictly worse than the unpatched bug (parser updates state but serving-side predicate not extended → state goes stale; or vice versa → predicate fires on stale state and the rescue computes a nonsense final delta). The patch refuses install on any landmark mismatch with `StreamingTruncationPatchRefusedError` naming the failing landmark.

**Removal trigger**: vLLM upstream lands a fix that EITHER (a) makes the parser's break path emit the partial fragment, OR (b) extends `_should_check_for_unstreamed_tool_arg_tokens` to fire on the truncation case AND adjusts the rescue body to be tolerant of `delta_message.tool_calls` being empty. The bug-shape landmark is the `# Parameter incomplete — break` comment (and the `break` it precedes) at parser line 569-573; any rewrite that emits before breaking changes its body and forces this patch to refuse. The rescue-body landmark is the unconditional `delta_message.tool_calls[0].function.arguments` access in `chat_completion_stream_generator`; if upstream guards it with `if delta_message.tool_calls`, the predicate wrap's mutation becomes redundant.

### 7.12 Patch 12 — `monkey_patch_repetition_detection_default.py`

Wraps `ChatCompletionRequest.to_sampling_params` (same target as patch 6, composes via the `__wrapped_original__` chain) and sets `SamplingParams.repetition_detection = RepetitionDetectionParams(max_pattern_size=8, min_pattern_size=1, min_count=96)` iff `"repetition_detection" not in request.model_fields_set`. vLLM PR #35451 (merged 2026-03-03) shipped n-gram repetition detection as a per-request opt-in via `RepetitionDetectionParams` in `vllm/sampling_params.py:110-145`; the V1 scheduler consumes it in `vllm/v1/core/sched/utils.py:check_stop` (line 119-128) and terminates generation with `stop_reason="repetition_detected"`. The defense addresses Qwen3.x's documented single-token-loop pathology (Qwen3.6 issue #145, cyankiwi HF #2 reproducing at full BF16, agent_service repo's May 2026 wall-of-zeros incident: model emitted 32,768 literal `0` characters mid-`write_file` tool call when encoding `2·10^50`).

A monkey patch is necessary because vLLM has no built-in mechanism to default this field server-side: `vllm/config/model.py:1427`'s `available_params` allow-list filters `repetition_detection` out of `default_sampling_params` (so `--override-generation-config '{"repetition_detection":...}'` is silently dropped), and `vllm/entrypoints/openai/chat_completion/protocol.py:596` passes `self.repetition_detection` straight through without consulting `default_sampling_params.get(...)` like it does for `temperature`. Both `model.py` and `protocol.py` are byte-identical between the pinned commit and master at the time of writing.

`min_count=96` is chosen so legitimate ≤95-digit decimal literals can complete (int64 ≈19 digits, int128 ≈39 digits, 256-bit integers up to 78 digits; the Qwen3 tokenizer emits each digit as a separate token) while bounding the worst-case degenerate burst at ~96 wasted tokens (~0.3 % of a 32 768-token response). `max_pattern_size=8` covers cycles up to 8-token n-grams; `min_pattern_size=1` covers the single-token wall-of-zeros mode specifically. Phase 8 runs three behavioural cases at import time: (a) no-fields-set → defense triple applied; (b) explicit `repetition_detection` preserved verbatim (negative control); (c) two separate calls produce non-aliasing `RepetitionDetectionParams` instances (mutation-leak guard). The wrapper composes with patch 6: each call invokes the `__wrapped_original__` chain inner→outer, both sets of defaults apply, both respect `model_fields_set`. Listed AFTER patch 6 in `_PATCH_MODULES` so this wrapper is the OUTERMOST `to_sampling_params` and the boot-log sequence reads "Qwen3.6 vendor defaults installed → repetition-detection defense installed" inner→outer.

**Removal trigger**: vLLM widens the `available_params` allow-list to include `repetition_detection` AND extends `to_sampling_params` to consult `default_sampling_params` for it (a ~5-line two-site change), at which point `--override-generation-config` becomes the principled mechanism. Alternatively, Alibaba updates the Qwen3.6 Best Practices block to recommend a `repetition_detection` triple, at which point patch 6 absorbs the field and this patch is removed.
...
Verifiers split into two classes. Patches 2 and 3 carry **behavioural** verifiers — they instantiate the patched class with a synthetic input designed to expose the bug and assert the post-patch return value. Patches 1, 4, 5, 6, 7, 8, 9, 10, 11, 12 carry tag-only launcher verifiers (with `getattr` and `inspect.getattr_static` agreement); their patch-internal Phase verifications carry the load-bearing functional verification (the egress patch's wire-dump check; the ingest patch's static-lookup check; patch 6's five behavioural cases at Phase 7; patch 7's four behavioural gate cases at Phase 9 (auto installs / empty-tools no-op / explicit-`structured_outputs` no-op / `tool_choice="none"` no-op) plus a legacy-shape rejection check, modern-shape adherence assertions including `style="qwen_xml"` per tag, a `xgr.Grammar.from_structural_tag(spec_str)` build-time compile, AND a load-bearing functional regression guard that constructs a synthetic `xgr.GrammarCompiler` over a 256-byte vocab, byte-walks the canonical `<tool_call>\n<function=calculator>\n<parameter=expr>\n2+2\n</parameter>\n</function>\n</tool_call>` and asserts every byte is accepted, then walks to the body-open boundary and asserts `{` is rejected — proving the FSM enforces the qwen_xml body branch, the v2 contract; patch 8's three behavioural probes — original-raises-on-buggy-input, patched-accepts-on-same-input, patched-still-raises-on-external-pressure; patch 9's five behavioural probes at Phase 4 directly testing the pure filter function — text+image preserves order, image-only preserves single element, text-only returns None (no intervention), string content returns None, and unknown-type filtering with both no-media-passthrough and media-present-filter sub-cases; patch 10's five harness cases at Phase 7 driving the wrapper via `asyncio.run` against a stub `OpenAIServingChat` — `ValueError` triggers eviction, `VLLMValidationError` triggers eviction, `RuntimeError` does NOT trigger eviction, happy-path passes through with no eviction, missing-renderer defensively skips eviction without crashing; patch 11's four behavioural cases at Phase 10 driving both wraps — case A 5-chunk truncation sequence through the patched parser asserts file_path AND partial-content bytes (`"import os"` / `"import sys"`) end up in `prev_tool_call_arr[-1]["arguments"]` with no spurious closing brace, case B re-runs the same chunks through the captured original to prove the unpatched parser drops the partial (precondition guard against silently-fixed-upstream regressions), case C drives the predicate wrap through original-True-passthrough / truncation-rescue-with-mutation / other-parser-class-no-fire / no-finish_reason-no-fire, case D verifies the end-to-end rescue subtraction `expected.replace(actual, "", 1)` yields exactly the missing-tail bytes), so duplicating it here would only double the surface area. Patch 7's launcher verifier additionally asserts `Qwen3CoderToolParser.supports_required_and_named is False`. Patch 11's launcher verifier checks BOTH wrap targets carry the tag — a half-installed truncation rescue is strictly worse than the unpatched bug.
...
vLLM v1 spawns EngineCore as a `multiprocessing` child process via `spawn` (CUDA forbids `fork` after init). The spawned interpreter does not inherit `sys.modules`. Of the twelve patches, **patches 3 and 8** target EngineCore-resident code (`monkey_patch_hybrid_kv_allocator` for the KV-cache reporting path, `monkey_patch_request_memory_snapshot` for the worker's startup `request_memory` check); patches 1, 2, 4, 5, 6, 7, 9, 10, 11, 12 target API-server-resident code and become live in PID 1 directly (chat_utils' `parse_chat_messages`, `OpenAIServingChat.create_chat_completion`, `OpenAIServingChat._should_check_for_unstreamed_tool_arg_tokens`, and `Qwen3CoderToolParser.extract_tool_calls_streaming` all run in the request handler, never in EngineCore). Without `sitecustomize`, patches 3 and 8 are silently dead in EngineCore while the launcher's PID-1 verifier reports success. The pass/fail discriminator for patch 3 is the boot-log filename annotation: `[kv_cache_utils.py:NNN]` (unpatched, ~37K tokens) vs `[monkey_patch_hybrid_kv_allocator.py:NNN]` (patched, ~149K tokens). For patch 8 the discriminator is whether the boot ValueError fires at gmu > ~0.984 (unpatched, snapshot-check too strict) or the engine reaches "Application startup complete" (patched).
...
**Load-bearing for**: patches 3 and 8. **Defense-in-depth for**: patches 1, 2, 4, 5, 6, 7, 9, 10, 11, 12. **Removal trigger**: when both patches 3 and 8 are removed; recommendation is to keep for defense-in-depth.
...
[`install.sh`](install.sh) takes one **required** positional argument: the host home directory whose `.cache/` subdirectory is bind-mounted at `/root/.cache/` inside the container. **Only `.cache/` is exposed**; the rest of the home directory (dotfiles, `.ssh`, `.gnupg`, `.aws`, `.docker`, Dockerfiles, `.env`, `.git-credentials`, Documents, projects) stays sealed off. This makes everything the engine writes — HuggingFace model weights (~14 GiB for QuantTrio/Qwen3.6-27B-AWQ), vLLM Triton compile cache (~355 MB), FlashInfer JIT kernel cache, pip cache — persistent across container restarts at zero re-download / re-compile cost. There is no default — `sudo` clobbers `HOME=/root` and silently binding `/root/.cache` would lose every cache on every fresh install. The script refuses to start without the explicit path; both `<home-dir>` and `<home-dir>/.cache/` must already exist.
...
**There is no `Dockerfile` and no `docker build` step in this repo.** The deployment runs the upstream `vllm/vllm-openai` image **unmodified**, and the eleven patches (plus `sitecustomize.py` and `launch_with_patches.py`) are bind-mounted into the container at `docker run` time via the `-v` flags in [`install.sh`](install.sh). The container's default `["vllm", "serve"]` entrypoint is replaced with `python3 /opt/patches/launch.py serve ...` so the launcher imports every patch — fail-loud on any landmark mismatch — *before* handing off to vLLM's CLI via `runpy`.
...
# Step 4: confirm all 11 distinct patch tags applied. Each patch loads in
...
# line count is ~33; dedupe to count distinct tags.
sudo docker logs qwen36 2>&1 | grep -oE 'qwen36-agent-setup-[a-z0-9-]+' | sort -u | wc -l   # expect 11
```
...
# valid JSON arguments serialised by the qwen3_coder XML parser from the
# model's <parameter=expr>...</parameter> body output; finish_reason ==
# "tool_calls". With patch 7 installed, xgrammar's structural_tag pins
# the function name AND enforces the per-tool parameter body via
# JSONSchemaFormat(style="qwen_xml") — hallucinated names cannot reach
# the wire, empty bodies are rejected at decode, and missing-required
# fields fail the FSM.
...
- [ ] `sudo docker logs qwen36 2>&1 | grep -oE 'qwen36-agent-setup-[a-z0-9-]+' | sort -u | wc -l` returns **11** (the eleven distinct patch tags). Patches load in three processes — PID 1 launcher, the launcher's pre-flight subprocess install probe, and the spawned EngineCore — so the un-deduped `applied:` line count is **~33**.
...
- [ ] §8.3 step 6 smoke test returns `finish_reason == "tool_calls"` with `tool_calls[0].function.name == "calculator"` AND `tool_calls[0].function.arguments` populated with a non-empty JSON object that satisfies the tool's schema (e.g. `{"expr": "127 * 349"}`). Patch §7.7 v2 enforces both function-name pinning AND parameter-body shape via xgrammar's `JSONSchemaFormat(style="qwen_xml")` — hallucinated names, empty bodies, JSON-shaped bodies, and missing-`required` parameters are all structurally unreachable at decode time. If the smoke test returns `arguments == "{}"`, the patch is operating in the v1-shaped legacy mode (regression); inspect `docker logs qwen36 | grep qwen36-agent-setup-qwen3-coder-grammar` and confirm the tag is `…-v2` and Phase 9's byte-walk regression guard reported pass.
...
210-prompt corpus served 2026-04-25 (Hebrew + ancient-Egyptian + emoji, 0% true garbling). Production §8.2 flags exercised against container `qwen36` on `127.0.0.1:8001` over 2026-04-27 (B4/B6/B7 close, §8.2 baseline) and 2026-04-28 (high-context expansion to 131K).
...
| **B13** | Thinking-mode toggle ground truth (2026-04-29) | **Toggle is empirically strict at production T=0.6 when wired correctly** | 95-trial sweep at T=0.6 with **top-level** `chat_template_kwargs.enable_thinking=false`: **0 leaks** across (a) 30 trials matrix of easy/medium/hard difficulties × T=0/T=0.6, (b) 30 trials prompt-length sweep at 5K/30K/60K/100K/130K/150K targets (132,618 actual tokens at the 150K cell), (c) 20 trials math/code/retrieval intents including a 148K-token RULER single-needle. Cross-engine bug citations re-examined: vllm #30477 was a user-error (wrong `extra_body` wire shape), #40816 is a streaming-only bug on a different model size, sglang/llama.cpp issues are engine-specific. **Critical wire-shape finding**: vLLM's request schema has `extra="allow"`, so `extra_body={"chat_template_kwargs": {...}}` in raw HTTP is silently dropped — the OpenAI Python SDK unwraps `extra_body` client-side, but raw `curl` does not. Payloads using the `extra_body`-nested shape are silent no-ops over raw HTTP; the apparent "leakage" such payloads show is the model thinking under the un-overridden server default and the parser routing `max_tokens`-truncated thinking (no `</think>`) to `reasoning_content` per its documented contract at `vllm/reasoning/qwen3_reasoning_parser.py:75-93`. |
| **B14** | Patch 9 (tool-role media preserve) live empirical confirmation (2026-04-29) | **Validated end-to-end** | Sent a 4-turn conversation `system → user → assistant(tool_calls) → tool` with the `tool` message's content as a 2-element list `[{type:text}, {type:image_url}]` carrying `disc5_imgs/img1.png` (793×224, ~155 vision tokens). HTTP 200; model response **"Query=Hi!, Reasoning panel, AI assistant interface"** (accurately describes the screenshot). **Smoking gun: 164-token prompt-token delta** between list-with-image (258 prompt tokens) and a plain-string negative control (94 prompt tokens). The 164-token delta matches the expected ~155 vision tokens for a 793×224 image plus framing — direct, unambiguous evidence that the image data flowed through the multimodal renderer despite being inside a `role:"tool"` message. Plain-string control hallucinates an empty UI, no vision tokens. Engine `/health=200` throughout. |
| **B15** | `add_vision_id` decision + content-format autodetect interaction (2026-04-29) | **Not training-load-bearing AND inert on this deployment until `content_format='openai'` is set** | (i) Training-side: Qwen3.6-VL model card L690-691 frames `add_vision_id` as opt-in convenience ("helpful to add labels … users can control this behavior"); canonical default example L725-728 omits it; the technical report does not mention the flag — flipping the server default is not training-fidelity-required. (ii) Live wire test: per-request `chat_template_kwargs.add_vision_id=true` (top-level, the B13 shape) produces token sequences **byte-identical** to the server default for 1/2/3-image requests via both `/tokenize` and `/v1/chat/completions`; full-prompt decode shows no `Picture N: ` prefix in either shape. n=1 multi-image ordinal probe (`THIRD image only` → `BLUE TEST`): both shapes correctly identify the third image; identical prompt_tokens (488). The override is silently dropped. (iii) Root cause: vLLM `_detect_content_format` (`renderers/hf.py:230-244`) walks the chat-template AST for top-level `for X in message['content']` loops; our `chat_template.jinja:6-7` performs the iteration inside the `render_content` macro and the detector misses it, returning `'string'` (`hf.py:271`; boot log `[hf.py:314] Detected the chat template content format to be 'string'`). Under `'string'`, `chat_utils.py:1382-1421` flattens multi-image content to a single string before the template runs, bypassing the `add_vision_id` branch (`chat_template.jinja:6-18`). To activate `add_vision_id`, add `--chat-template-content-format openai` to §8.2 (also makes `chat_template_kwargs.add_vision_id` per-request toggle work). **Decision: leave `add_vision_id` off** — opt-in by training, n=1 ordinal probe shows the model resolves multi-image grounding correctly without it, and the cost (~5 extra text tokens per image, total token budget) is unjustified for our workload mix. |
| **B16** | 130K–152K needle-in-haystack gap closure at depths 10/50/90 (2026-04-29) | **9/9 PASS verbatim recall** | 9-cell matrix at temperature 0, top-level `chat_template_kwargs.enable_thinking=false`, `max_tokens=60`, fresh per-trial random suffixes, deterministic `[entry NNNNNN] <topic>` filler over a 12-topic Qwen-stack pool. Three lengths × three depths: `L130K_D{10,50,90}` (actual prompt 129 903–130 136), `L145K_D{10,50,90}` (144 925–145 071), `L152K_D{10,50,90}` (151 044–151 074). All 9 cells return verbatim `The secret token is XYZ123-<suffix>` with `finish_reason=stop`. Latencies 50–62 s per cell (130K cold ~50 s, 145K ~59 s, 151K ~62 s); engine `/health=200` throughout. The `L152K` cells calibrate to 151 000 user-text tokens — chat-template framing pushes the final input to ~99.4% of the 152 000 cap; targeting 152 000 directly tips framed-prompt + `max_tokens=60` over the cap per `renderers/params.py:411-425` (B9-(e)). Complements B12's single-needle 148 337-token cold-prefill probe with depth-stratified coverage. |
| **B17** | `preserve_thinking=true` source citations (2026-04-29) | **Qwen-canonical, opt-in for agent scenarios; no published controlled ablation** | Qwen3.6-27B HF model card L811: *"additionally trained to preserve and leverage thinking traces from historical messages"*; L839: *"particularly beneficial for agent scenarios"*; L869: Qwen-Agent reference deployment sets `preserve_thinking=True` for DashScope and vLLM/SGLang. Predecessor Qwen3 (Qwen3-32B model card "Best Practices" → "No Thinking Content in History") explicitly recommended STRIPPING `<think>` from history — Qwen3.6 is a deliberate inversion. The only public empirical claim of degradation without the flag is `badlogic/pi-mono#3325` (single user, Qwen3.6-35B-A3B + LM Studio, empty-arg tool calls after 2–3 turns); two follow-ups (`badlogic/pi-mono#3479`, `jundot/omlx#900`) report `preserve_thinking=true` necessary but not always sufficient. vLLM `--default-chat-template-kwargs` was introduced as a generic feature by PR #31343 (commit `dc837bc23`) predating Qwen3.6 — the recommendation is Qwen-canonical, not vLLM-canonical. |
| **B18** | Patches × streaming-mode coverage audit + partial-marker bug-claim disposal (2026-04-29) | **Only §7.5 has a streaming gap; partial-marker concerns dismissed as theoretical-only** | (i) Patch-by-patch streaming-coverage source-grounded audit at the pinned vLLM commit `8cd174fa3`: of the 10 patches, 7 are NO_STREAMING_AXIS (run once per request before generation, or boot-time only — §7.1 / §7.3 / §7.6 / §7.7 / §7.8 / §7.9 plus the launcher §7.L); 2 are STREAMING_COVERED via shared primitive (§7.2's `_parse_xml_function_call` is called from `extract_tool_calls_streaming` at `vllm/tool_parsers/qwen3coder_tool_parser.py:643`; §7.4 explicitly patches both `ChatMessage` (full) AND `DeltaMessage` (streaming) in `_TARGETS` and verifies via Phase 3 dual `model_dump_json` exercise); 1 is STREAMING_COVERED via above-the-fork wrapper (§7.10 wraps `create_chat_completion` which contains the stream/full fork at `vllm/entrypoints/openai/chat_completion/serving.py:357`); 1 (§7.5) was the only outstanding STREAMING_GAP at the time of audit — subsequently closed by adding a sibling streaming wrapper that funnels through the same `_WARNING_FORMAT` constant as the non-streaming wrapper. (ii) Tokenizer + parser audit of two HIGH-severity partial-marker bug hypotheses, via direct `tk.encode()` calls on the `QuantTrio/Qwen3.6-27B-AWQ` tokenizer. **`</think>` split across deltas** → THEORETICAL_ONLY: `</think>` is added_token id 248069, ALWAYS encoded as a single atomic token; vLLM's incremental detokenizer (`vllm/v1/engine/detokenizer.py:117-119`, `vllm/tokenizers/detokenizer_utils.py:194-202`) emits the complete decoded string per token; the parser's trigger at `vllm/reasoning/qwen3_reasoning_parser.py:123-136` is on token-id `self.end_token_id in delta_token_ids`, not substring, so when the trigger fires `delta_text.find("</think>")` is guaranteed to succeed. **`</parameter>` split across deltas** → THEORETICAL_ONLY (different reason): `</parameter>` IS multi-token (`[510, 15704, 29]` = `</`, `parameter`, `>`) and DOES arrive as 3 separate deltas at the wire, BUT the parser searches CUMULATIVE `tool_text` (`previous_text + delta_text`) at `vllm/tool_parsers/qwen3coder_tool_parser.py:550`, not `delta_text`, and `param_count += 1` (line 603) only fires AFTER a successful emit at line 604 — so partial markers cause a clean break-and-retry without advancing the counter, with the parameter delivered correctly one delta later. Same break-and-retry pattern handles the `<function=NAME>` and `<parameter=NAME>` openers. |
...
16. **vLLM upstream guards `chat_completion/serving.py:823` and `:1407` on `is_mistral_tool_parser(type(tool_parser))`** — the existing correct pattern at `engine/serving.py:619`. Closes §6.10 properly. When that lands, setting `_grammar_from_tool_parser=True` from a non-Mistral parser becomes safe again, but our §7.7 patch still won't set it (Mistral helpers are semantically wrong for Qwen3 output regardless of dispatch safety). Re-audit Phase 9 case 1's `is False` assertion at that point.
17. **vLLM upstream lands a fix for the D13 streaming-truncation byte-loss** — either (a) makes `Qwen3CoderToolParser.extract_tool_calls_streaming`'s break path at `qwen3coder_tool_parser.py:569-573` emit the partial fragment before breaking, OR (b) extends `OpenAIServingChat._should_check_for_unstreamed_tool_arg_tokens` to fire when the parser returned None on a final delta AND adjusts the rescue body's `delta_message.tool_calls[0]` indexing at `serving.py:1116-1126` to be empty-tolerant. The bug-shape landmark on the parser side is the `# Parameter incomplete — break` comment (above the `break` at line 573); the rescue-body landmark is the unconditional `delta_message.tool_calls[0].function.arguments` access. Either rewrite changes one of those landmarks and forces patch 11 to refuse — remove patch 11.
...
├── launch_with_patches.py                                 # §7.L — container entrypoint; imports the 11 patches then runpys vLLM
...
├── monkey_patch_qwen3_coder_streaming_truncation.py       # §7.11 — D13 streaming-mid-value-truncation rescue: parser-side wrap keeps prev_tool_call_arr in sync with the partial; serving-side wrap fires the rescue predicate on truncation and synthesises the skeleton DeltaToolCall the rescue body indexes
...
Thirteen Python files (11 patches + launcher + sitecustomize), two top-level shell scripts (`install.sh` / `uninstall.sh`), the `host_ops/` deep-probe bundle (one shell script + two systemd unit files + one installer + one uninstaller), and one test file.
