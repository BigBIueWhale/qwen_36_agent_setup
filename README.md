...
| **B18** | Patches × streaming-coverage audit + partial-marker bug-claim disposal (Sub-agents K + L) (2026-04-29) | **Only §7.5 has a streaming gap; Sub-agent I's two HIGH-severity partial-marker claims dismissed as theoretical-only** | Two parallel source-grounded audits at the pinned vLLM commit `8cd174fa3`. (i) **Sub-agent K** — patch-by-patch streaming-coverage audit. Of the 10 monkey-patches: 7 are NO_STREAMING_AXIS (run once per request before generation, or boot-time only — §7.1 / §7.3 / §7.6 / §7.7 / §7.8 / §7.9 plus the launcher §7.L); 2 are STREAMING_COVERED via shared primitive (§7.2's `_parse_xml_function_call` is called from `extract_tool_calls_streaming` at `vllm/tool_parsers/qwen3coder_tool_parser.py:643`; §7.4 explicitly patches both `ChatMessage` (full) AND `DeltaMessage` (streaming) in `_TARGETS` and verifies via Phase 3 dual `model_dump_json` exercise); 1 is STREAMING_COVERED via above-the-fork wrapper (§7.10 wraps `create_chat_completion` which contains the stream/full fork at `vllm/entrypoints/openai/chat_completion/serving.py:357`); 1 is STREAMING_GAP_INTENTIONAL (§7.5 — the docstring's "rate is a model-side property" justification re-confirmed but not empirically verified). (ii) **Sub-agent L** — tokenizer + parser audit of Sub-agent I's two HIGH-severity claims via direct `tk.encode()` calls on the `QuantTrio/Qwen3.6-27B-AWQ` tokenizer (snapshot `9b507bdc9afafb87b7898700cc2a591aa6639461`). **Claim A** (`</think>` split across deltas) → THEORETICAL_ONLY: `</think>` is added_token id 248069, ALWAYS encoded as a single atomic token; vLLM's incremental detokenizer (`vllm/v1/engine/detokenizer.py:117-119`, `vllm/tokenizers/detokenizer_utils.py:194-202`) emits the complete decoded string per token; the parser's trigger at `vllm/reasoning/qwen3_reasoning_parser.py:123-136` is on token-ID `self.end_token_id in delta_token_ids`, not substring — when the trigger fires, `delta_text.find("</think>")` is guaranteed to succeed. **Claim B** (`</parameter>` split) → THEORETICAL_ONLY (different reason): `</parameter>` IS multi-token (`[510, 15704, 29]` = `</`, `parameter`, `>`) and DOES arrive as 3 separate deltas at the wire, BUT the parser searches CUMULATIVE `tool_text` (`previous_text + delta_text`) at `vllm/tool_parsers/qwen3coder_tool_parser.py:550`, not `delta_text`, and `param_count += 1` (line 603) only fires AFTER a successful emit at line 604 — so partial markers cause a clean break-and-retry without advancing the counter, with the parameter delivered correctly one delta later. Sub-agent I's specific failure model ("the parser is already past this parameter count, so the incomplete parameter is silently dropped") was source-incorrect. Decision: close tasks #82 (Claim A) and #83 (Claim B); task #84 (§7.5 streaming-sibling decision) stays open pending empirical rate-invariance verification — measure tool-call-in-reasoning rate across non-streaming probe vs streaming production traffic before deciding whether to add a `extract_reasoning_streaming` sibling wrapper. Bonus L finding: `<function=NAME>` and `<parameter=NAME>` openers are also multi-token but use the same break-and-retry pattern correctly. Sub-agent K + L findings inline in transcript; predecessor Sub-agent I report at `/tmp/qwen36_research/streaming_research_2026-04-29/`. |
