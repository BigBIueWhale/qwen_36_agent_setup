...
| L | [`launch_with_patches.py`](launch_with_patches.py) | Container entrypoint that imports the 11 patches in order, runs per-patch verification, then hands off to `vllm.entrypoints.cli.main` via `runpy.run_module(alter_sys=True)` |
...
**Load-bearing for**: patches 3 and 8. **Defense-in-depth for**: patches 1, 2, 4, 5, 6, 7, 9, 10, 11. **Removal trigger**: when both patches 3 and 8 are removed; recommendation is to keep for defense-in-depth.
